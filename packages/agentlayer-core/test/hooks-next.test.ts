...
describe('preToolUse — ctx.next(mutatedInput)', () => {
	...
	test('state mutation does not change next(updatedInput) behavior', async () => {
		let receivedInput: Record<string, unknown> | null = null
		let seenInSecondHook: number | undefined
		...
		const hook1: PreToolUseHook = (ctx) => {
			...
			return ctx.next({ ...ctx.input, text: 'hooked' })
		}
		const hook2: PreToolUseHook = (ctx) => {
			seenInSecondHook = ctx.getState<number>('seen')
			return ctx.next({ ...ctx.input, text: `${(ctx.input as any).text}-again` })
		}
		...
		expect((receivedInput as any)?.text).toBe('hooked-again')
		expect(seenInSecondHook).toBe(1)
		expect(result.state.toolState).toEqual({ seen: 1 })
	})

	test('state mutation does not change next options aggregation behavior', async () => {
		const echoTool = defineTool({
			name: 'echo',
			description: 'Echoes input',
			input: z.object({ text: z.string() }),
		...
		const hook1: PreToolUseHook = (ctx) => {
			...
			ctx.updateState('count', () => count + 1)
			return ctx.next({ ...ctx.input, text: 'step1' }, { notifyModel: true })
		}
		const hook2: PreToolUseHook = (ctx) => {
			const count = ctx.getState<number>('count') ?? 0
			ctx.updateState('count', () => count + 1)
			return ctx.next({ ...ctx.input, text: 'step2' }, { updateContextWindow: true })
		}
		...
		const toolResult = (toolMsg!.content as any[])[0]
		expect(toolResult.output.value).toContain('<system_information>')
		expect(toolResult.output.value).toContain('step2')
		expect(result.state.toolState).toEqual({ count: 2 })
	})
})
