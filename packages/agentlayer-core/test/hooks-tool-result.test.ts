...
describe('preToolUse — ctx.toolResult()', () => {
	...
	test('toolResult with isError: true marks result as error', async () => {
		...
		const validationHook: PreToolUseHook = (ctx) =>
			ctx.toolResult('Validation failed. Please fix and try again.', { isError: true })

		const agent = new Agent({
			model: mockModel([assistantWithToolCall('done', {}), assistantText('OK.')]),
			tools: { done: doneTool },
		...
		const [toolResultPart] = getToolResults(result.state.messages)
		expect(toolResultPart).toBeDefined()
		expect(outputValue(toolResultPart!)).toBe('Validation failed. Please fix and try again.')
		expect(toolResultPart!.isError).toBe(true)
	})

	test('toolResult defaults to isError: false when no options provided', async () => {
		const doneTool = defineTool({
			name: 'done',
			description: 'Signal completion',
			input: z.object({}),
