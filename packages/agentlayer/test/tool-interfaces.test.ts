...
describe('All interfaces produce valid tools via .define()', () => {
	test('BashTool.define() returns a tool named "bash"', () => {
		const tool = BashTool.define(async () => 'ok')
	...
	test('WebFetchTool.define() returns a tool named "webfetch"', () => {
		const tool = WebFetchTool.define(async () => 'content')
		expect(tool.name).toBe('webfetch')
		expect(tool.description.length).toBeGreaterThan(0)
	})

	test('WebSearchTool.define() returns a tool named "websearch"', () => {
		const tool = WebSearchTool.define(async () => ({ results: [] }))
		expect(tool.name).toBe('websearch')
		expect(tool.description.length).toBeGreaterThan(0)
	})

	test('CodeSearchTool.define() returns a tool named "codesearch"', () => {
...
describe('Zod schemas — valid / invalid inputs', () => {
	// ── editInput ──
	test('editInput requires filePath, oldString, newString', () => {
		expect(editInput.safeParse({}).success).toBe(false)
		expect(editInput.safeParse({ filePath: '/f' }).success).toBe(false)
		expect(editInput.safeParse({ filePath: '/f', oldString: 'a' }).success).toBe(false)
	})

	test('editInput parses valid input with defaults', () => {
		const result = editInput.safeParse({ filePath: '/f', oldString: 'a', newString: 'b' })
		expect(result.success).toBe(true)
		if (result.success) {
			expect(result.data.replaceAll).toBe(false)
		}
	})

	test('editInput accepts replaceAll=true', () => {
	...
	// ── writeInput ──
	test('writeInput requires filePath and content', () => {
		expect(writeInput.safeParse({}).success).toBe(false)
		expect(writeInput.safeParse({ filePath: '/f' }).success).toBe(false)
	})

	test('writeInput parses valid input', () => {
		const result = writeInput.safeParse({ filePath: '/f', content: 'hello' })
		expect(result.success).toBe(true)
	})

	test('writeInput accepts empty content (for creating empty files)', () => {
		const result = writeInput.safeParse({ filePath: '/f', content: '' })
		expect(result.success).toBe(true)
	})

	// ── multiEditInput ──
	test('multiEditInput requires at least one edit', () => {
		expect(multiEditInput.safeParse({ filePath: '/f', edits: [] }).success).toBe(false)
	})
	...
	test('multiEditInput parses valid input with multiple edits', () => {
		...
		if (result.success) {
			...
			expect(result.data.edits[0]!.replaceAll).toBe(false) // default
			expect(result.data.edits[1]!.replaceAll).toBe(true)
		}
	})
	...
	// ── applyPatchInput ──
	test('applyPatchInput requires patchText', () => {
	...
	// ── webFetchInput ──
	test('webFetchInput requires url', () => {
		expect(webFetchInput.safeParse({}).success).toBe(false)
	})

	test('webFetchInput rejects invalid URL', () => {
		expect(webFetchInput.safeParse({ url: 'not-a-url' }).success).toBe(false)
		// Note: Zod's z.string().url() accepts any scheme; http/https enforcement is in executor
		expect(webFetchInput.safeParse({ url: 'relative/path' }).success).toBe(false)
	})

	test('webFetchInput parses valid URL with defaults', () => {
		const result = webFetchInput.safeParse({ url: 'https://example.com' })
		expect(result.success).toBe(true)
		if (result.success) {
	...
	test('webFetchInput accepts format and timeout overrides', () => {
	...
	test('webFetchInput accepts all format values', () => {
		for (const format of ['text', 'markdown', 'html'] as const) {
			const result = webFetchInput.safeParse({ url: 'https://example.com', format })
			expect(result.success).toBe(true)
		}
	})

	test('webFetchInput rejects invalid format', () => {
		const result = webFetchInput.safeParse({ url: 'https://example.com', format: 'json' })
		expect(result.success).toBe(false)
	})

	// ── webSearchInput ──
	test('webSearchInput requires query', () => {
		expect(webSearchInput.safeParse({}).success).toBe(false)
	})

	test('webSearchInput parses valid input with defaults', () => {
		const result = webSearchInput.safeParse({ query: 'bun test runner' })
		expect(result.success).toBe(true)
		if (result.success) {
	...
	test('webSearchInput accepts numResults override', () => {
		const result = webSearchInput.safeParse({ query: 'bun', numResults: 10 })
		expect(result.success).toBe(true)
		if (result.success) {
	...
	// ── codeSearchInput ──
	test('codeSearchInput requires query and packageName', () => {
	...
	test('codeSearchInput parses valid input with language defaulting to typescript', () => {
	...
	test('codeSearchInput accepts custom language', () => {
		const result = codeSearchInput.safeParse({ query: 'arrays', packageName: 'numpy', language: 'python' })
		expect(result.success).toBe(true)
		if (result.success) {
...
describe('EditTool serialize — match count handling', () => {
	test('returns success message when matchCount > 0', async () => {
		...
		const input = { filePath: '/test/file.ts', oldString: 'old', newString: 'new', replaceAll: false }
	...
	test('returns success for multiple matches', async () => {
		...
		const input = { filePath: '/test/file.ts', oldString: 'old', newString: 'new', replaceAll: true }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
		expect(output).toContain('Successfully edited')
	})
	...
	test('returns error message when matchCount is 0', async () => {
		...
		const input = { filePath: '/test/file.ts', oldString: 'missing', newString: 'new', replaceAll: false }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
	...
	test('uses original filePath in error message', async () => {
		const tool = EditTool.define(async () => ({ content: 'unchanged', matchCount: 0 }))
		const input = { filePath: '~/project/file.ts', oldString: 'missing', newString: 'new', replaceAll: false }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
...
describe('WriteTool content passthrough', () => {
	test('passes content unchanged to executor', async () => {
		...
		await tool.execute({ filePath: '/test/file.ts', content: multiLineContent }, makeToolContext())
	...
	test('passes empty content to executor', async () => {
		...
		await tool.execute({ filePath: '/test/empty.ts', content: '' }, makeToolContext())
	...
	test('preserves CRLF line endings in content', async () => {
		...
		await tool.execute({ filePath: '/test/crlf.ts', content: crlfContent }, makeToolContext())
...
describe('MultiEditTool passthrough', () => {
	test('passes edits array to executor unchanged', async () => {
		...
		await tool.execute({ filePath: '/test/file.ts', edits }, makeToolContext())
...
describe('ApplyPatchTool', () => {
	test('passes patchText directly to executor', async () => {
		let receivedPatchText = ''
		const tool = ApplyPatchTool.define(async (input) => {
			receivedPatchText = input.patchText
			return 'Patch applied successfully'
		})
		const patchText = '*** Begin Patch\n*** Add File: test.txt\n+hello world\n*** End Patch'
		const input = { patchText }
		const raw = await tool.execute(input, makeToolContext())
		expect(receivedPatchText).toBe(patchText)
		const output = serializeRaw(tool, raw as any, input)
	...
	test('executor error propagates', async () => {
		...
		await expect(tool.execute({ patchText: 'invalid' }, makeToolContext())).rejects.toThrow(
	...
	test('multiline patch text is passed correctly', async () => {
		...
		await tool.execute({ patchText: multilinePatch }, makeToolContext())
	...
	test('heredoc-wrapped patch text is passed as-is (no unwrapping in interface)', async () => {
		...
		await tool.execute({ patchText: heredocPatch }, makeToolContext())
...
describe('EditTool — simulated executor scenarios', () => {
	test('single match replacement', async () => {
		...
		const input = { filePath: '/test.ts', oldString: 'old content', newString: 'new content', replaceAll: false }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
		expect(output).toContain('Successfully edited')
	})
	...
	test('replaceAll with multiple matches', async () => {
		const tool = EditTool.define(async (input) => {
			...
			const regex = new RegExp(input.oldString.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')
			const matches = content.match(regex)
			const matchCount = matches?.length ?? 0
			const updated = content.replace(regex, input.newString)
			return { content: updated, matchCount }
		})

		const input = { filePath: '/test.ts', oldString: 'foo', newString: 'qux', replaceAll: true }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
		expect(output).toContain('Successfully edited')
	})
	...
	test('no match returns error', async () => {
		...
		const input = { filePath: '/test.ts', oldString: 'does not exist', newString: 'new', replaceAll: false }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
		expect(output).toContain('Error')
		expect(output).toContain('Could not find a match')
	...
	test('executor throw propagates to caller', async () => {
		...
		await expect(
			tool.execute(
				{ filePath: '/missing.ts', oldString: 'a', newString: 'b', replaceAll: false },
				makeToolContext(),
			),
...
describe('WriteTool — simulated executor scenarios', () => {
	test('new file creation returns executor output', async () => {
		const tool = WriteTool.define(async (input) => `Created ${input.filePath}`)
		const input = { filePath: '/new-file.ts', content: 'export {}' }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
	...
	test('overwriting file returns executor output', async () => {
		const tool = WriteTool.define(async (input) => `Wrote ${input.content.length} bytes to ${input.filePath}`)
		const input = { filePath: '/existing.ts', content: 'new content here' }
		const raw = await tool.execute(input, makeToolContext())
		const output = serializeRaw(tool, raw as any, input)
	...
	test('JSON content is passed correctly', async () => {
		...
		await tool.execute({ filePath: '/data.json', content: json }, makeToolContext())
	...
	test('executor error propagates', async () => {
		...
		await expect(tool.execute({ filePath: '/readonly/file.ts', content: 'x' }, makeToolContext())).rejects.toThrow(
			'Permission denied',
		)
	})
})
...
describe('normalizeEscapes', () => {
	...
	test('converts literal \\n to newlines', async () => {
		const { normalizeEscapes } = await import('../src/tools/interfaces/edit')
