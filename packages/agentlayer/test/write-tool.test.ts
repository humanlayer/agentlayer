...
import { createWriteTool } from '../src/tools/server/write'
...
describe('createWriteTool', () => {
	test('creates a new file with given content', async () => {
		...
		try {
			...
			const result = await writeTool.execute({ filePath, content }, makeToolContext())
	...
	test('creates parent directories when they do not exist', async () => {
		...
		try {
			...
			const writeTool = createWriteTool()
			await writeTool.execute({ filePath, content }, makeToolContext())
	...
	test('overwrites existing file with new content', async () => {
		...
		try {
			...
			const writeTool = createWriteTool()
			await writeTool.execute({ filePath, content: originalContent }, makeToolContext())
			await writeTool.execute({ filePath, content: newContent }, makeToolContext())
	...
	test('preserves CRLF line endings', async () => {
		...
		try {
			...
			const writeTool = createWriteTool()
			await writeTool.execute({ filePath, content: crlfContent }, makeToolContext())
	...
	test('handles JSON content correctly', async () => {
		...
		try {
			...
			const writeTool = createWriteTool()
			await writeTool.execute({ filePath, content: json }, makeToolContext())
	...
	test('returns result containing the filePath', async () => {
		...
		try {
			...
			const writeTool = createWriteTool()
			const result = await writeTool.execute({ filePath, content: 'x' }, makeToolContext())
