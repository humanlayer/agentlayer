...
import { Agent, defineTool, getPendingToolCalls, startState, toolResultMessage, withApprovals } from '../src'
import {
	assistantText,
...
describe('omitting approvals — backwards compat auto-execution', () => {
	test('dangling tool calls auto-execute when approvals is omitted (backwards-compat)', async () => {
		...
		const { toolCalled } = await import('../src/core/stop-conditions')
...
describe.skipIf(!process.env.ANTHROPIC_API_KEY)('anthropic approval resume', () => {
