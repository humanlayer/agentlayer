...
import { getModelLimits, getModelPricing, initModelsCache } from '../models'
...
export class Agent<TTools extends Record<string, Tool<any, any>> = Record<string, Tool<any, any>>> {
	...
	constructor(config: AgentConfig<TTools>) {
		...
		this.maxStepsLimit = config.maxSteps ?? 50
	...
	private async executeLoop(options: RunOptions, agentRun: AgentRun): Promise<void> {
		...
		if (this.contextWindowLimit === undefined) {
			...
			const limits = getModelLimits(modelKey)
			if (limits) {
				this.contextWindowLimit = limits.context
			}
		}
		const accumulator = new TokenUsageAccumulator(getModelPricing)
		...
		try {
			...
			// ── main loop ───────────────────────────────────────────────────────
			for (let stepIndex = 0; stepIndex < this.maxStepsLimit; stepIndex++) {
				...
				// ── Merge tool state updates from all executed outcomes ────────────
	...
	private async resolveToolCall(
		...
		if (preToolUseHooks && preToolUseHooks.length > 0) {
			const hookResult = await runPreToolUseHooks(preToolUseHooks, {
				toolName: tc.toolName,
				toolCallId: tc.toolCallId,
				input: tcInput,
				tool: toolInfo,
			...
			if (hookResult.type === 'toolResult') {
				const message = buildToolResultMessage(tc.toolCallId, tc.toolName, hookResult.output, false)
				return {
					kind: 'toolResult',
					toolCallId: tc.toolCallId,
					toolName: tc.toolName,
			...
			const execResult = await this.executeWithPostHooks(mutatedTc, currentToolCtx, toolInfo, allMessages)
			return {
				kind: 'executed',
	...
	private async executeWithPostHooks(
		...
		const hookResult = await runPostToolUseHooks(postToolUseHooks, {
			toolName: tc.toolName,
			toolCallId: tc.toolCallId,
			input: tcInput,
			output: execResult.output,
		...
		if (hookResult.mutatedResult !== undefined) {
			execResult = {
				...execResult,
				output: hookResult.mutatedResult,
				message: buildToolResultMessage(tc.toolCallId, tc.toolName, hookResult.mutatedResult, false),
			}
		}
