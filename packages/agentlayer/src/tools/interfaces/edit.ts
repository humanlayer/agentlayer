...
export const EditTool = defineToolInterface<EditInput, EditOutput>({
	...
	serialize: (raw: EditOutput, input: EditInput) => {
		if (raw.matchCount === 0) {
			return `Error: Could not find a match for the provided oldString in ${input.filePath}. Make sure your oldString matches the file content exactly. Use the read tool to verify.`
		}
