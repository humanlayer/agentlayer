...
export function createExaWebSearchTool(opts: ExaWebSearchOptions) {
	...
	return WebSearchTool.define(
		async (input: WebSearchInput): Promise<WebSearchResult> => {
			try {
				return await fetchExaSearch(input.query, input.numResults, opts.exaApiKey, timeoutMs)
			} catch (err: unknown) {
...
export { createExaWebSearchTool as createWebSearchTool }
