...
import type { AgentEvent } from '../core/agent-run'
...
function shortPath(s: string): string {
	// "/Users/kyle/wt/synclayer/copilot-codex-providers/apps/foo.ts" -> "apps/foo.ts"
...
function compactInput(rawInput: unknown): string {
	...
	if (typeof rawInput === 'string') {
		try {
			...
			return truncate(rawInput, MAX_INPUT_VAL)
		}
	}
...
function _prettyTruncatedOutput(output: unknown): string {
	if (output == null) return ''
	if (typeof output === 'string') return truncate(output.trim(), MAX_OUTPUT)
	if (typeof output !== 'object') return String(output)
	const o = output as Record<string, unknown>
	if (o.type === 'text' && typeof o.value === 'string') return truncate(o.value.trim(), MAX_OUTPUT)
	if (o.type === 'json') return truncate(JSON.stringify(o.value), MAX_OUTPUT)
	if (o.type === 'error-text' && typeof o.value === 'string') return color.red(truncate(o.value.trim(), MAX_OUTPUT))
	if (o.type === 'error-json') return color.red(truncate(JSON.stringify(o.value), MAX_OUTPUT))
	if (o.type === 'execution-denied')
		return color.red(`denied${(o as { reason?: string }).reason ? `: ${(o as { reason: string }).reason}` : ''}`)
	return truncate(JSON.stringify(output), MAX_OUTPUT)
}

export interface RendererOptions {
	showResponse?: boolean
	/** 'bracket' = "[Tool] Name(...)", 'compact' = "Name(...)" */
	toolLabelStyle?: 'bracket' | 'compact'
}

export class Renderer {
	private depthMap = new Map<string, number>()
	private toolCallOwner = new Map<string, string>() // toolCallId -> agentId
		...
		if (msg.role === 'assistant') {
			...
			for (const part of parts) {
				if (part.type === 'tool-result') {
					...
					const out = _prettyTruncatedOutput(tr.output)
					if (isErr) {
...
function renderTokenUsage(usage: TokenUsage): void {
	...
	for (const [modelId, m] of models) {
		...
		const row = `  ${displayId.padEnd(colW)} ${fmt(m.inputTokens).padStart(8)} ${fmt(m.cacheReadTokens).padStart(8)} ${fmt(m.cacheWriteTokens).padStart(8)} ${fmt(m.outputTokens).padStart(8)} ${fmtCost(m.estimatedCostUsd).padStart(8)}`
