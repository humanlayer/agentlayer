						...
						const dispatched = yield* Effect.forEach(
							...
							(call) =>
								dispatch(tools, call).pipe(
									Effect.map((result) => [call, result.result, result.error] as const),
								),
							{ concurrency },
						)
...
const dispatch = (
	...
	if (!tool.execute)
		return Effect.succeed({
			result: { type: 'error' as const, value: `Tool has no execute handler: ${call.name}` },
		})
