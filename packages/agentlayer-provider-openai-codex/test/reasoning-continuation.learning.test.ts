...
describe.skipIf(!hasAuth)('reasoning continuation learning test', () => {
	...
	test('continuation WITHOUT id field - should succeed using encrypted_content only', async () => {
		const model = createCodexLanguageModel({
			modelId: 'gpt-5.5',
			authStore,
		})
		...
		// Build a continuation request — id fields are stripped at the request body level
		...
		// The reasoning item should NOT have an `id` field (stripped by buildCodexRequestBody)
		const reasoningInput = body.input.find((i: any) => i.type === 'reasoning')
		expect(reasoningInput).toBeDefined()
		expect(reasoningInput?.id).toBeUndefined()
		expect(reasoningInput?.encrypted_content).toBeDefined()

		// Now make the actual continuation request — should work without id
