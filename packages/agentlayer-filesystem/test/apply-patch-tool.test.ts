...
import { createApplyPatchTool } from '../src/tools'
...
describe('createApplyPatchTool', () => {
	...
	test('resolves relative paths against opts.cwd', async () => {
		...
		try {
			...
			await tool.execute({ patch_text: patch }, makeToolContext())

			const content = await readFile(join(dir, 'relative.ts'), 'utf-8')
			expect(content).toBe('const relative = true\n')
		} finally {
			await rm(dir, { recursive: true })
		}
	})

	test('throws when patch_text is empty', async () => {
		const tool = createApplyPatchTool()
		await expect(tool.execute({ patch_text: '' }, makeToolContext())).rejects.toThrow('patch_text is required')
	})

	test('throws when patch_text is whitespace only', async () => {
		const tool = createApplyPatchTool()
		await expect(tool.execute({ patch_text: '   \n  ' }, makeToolContext())).rejects.toThrow(
			'patch_text is required',
		)
	})

	test('throws on invalid patch format (missing markers)', async () => {
		const tool = createApplyPatchTool()
		await expect(tool.execute({ patch_text: 'this is not a patch' }, makeToolContext())).rejects.toThrow(
	...
	test('Add File: creates a new file with given content (+ prefix stripped)', async () => {
		...
		try {
			...
			const tool = createApplyPatchTool()
			const result = await tool.execute({ patch_text: patch }, makeToolContext())
	...
	test('Update File: throws when target file does not exist', async () => {
		...
		try {
			...
			const tool = createApplyPatchTool()
			await expect(tool.execute({ patch_text: patch }, makeToolContext())).rejects.toThrow(
				'apply_patch verification failed',
			)
		} finally {
			await rm(dir, { recursive: true })
	...
	test('rejects valid-but-empty patch (no operations)', async () => {
		const tool = createApplyPatchTool()
		await expect(tool.execute({ patch_text: '*** Begin Patch\n*** End Patch' }, makeToolContext())).rejects.toThrow(
	...
	test('rejects unknown hunk header type', async () => {
		const tool = createApplyPatchTool()
		await expect(
			tool.execute({ patch_text: '*** Begin Patch\n*** Frobnicate File: foo\n*** End Patch' }, makeToolContext()),
