...
export class OpenAICompatibleChatLanguageModel implements LanguageModelV3 {
	...
	private get providerOptionsName(): string {
		return this.config.provider.split('.')[0]?.trim() ?? this.config.provider
	}
	...
	async doGenerate(options: LanguageModelV3CallOptions) {
		...
		if (!choice) {
			throw new InvalidResponseDataError({
				data: rawResponse,
				message: 'Response must contain at least one choice.',
			})
		}
		...
		const modelMetadata = providerMetadata[this.providerOptionsName] ?? {}
		providerMetadata[this.providerOptionsName] = modelMetadata
		const completionTokenDetails = responseBody.usage?.completion_tokens_details
		if (completionTokenDetails?.accepted_prediction_tokens != null) {
			modelMetadata.acceptedPredictionTokens = completionTokenDetails?.accepted_prediction_tokens
		}
		if (completionTokenDetails?.rejected_prediction_tokens != null) {
			modelMetadata.rejectedPredictionTokens = completionTokenDetails?.rejected_prediction_tokens
		}
			...
			stream: response.pipeThrough(
				new TransformStream<ParseResult<z.infer<typeof this.chunkSchema>>, LanguageModelV3StreamPart>({
							...
							for (const toolCallDelta of delta.tool_calls) {
								...
								if (toolCalls[index] == null) {
									...
									if (!toolCall) {
										throw new InvalidResponseDataError({
											data: toolCallDelta,
											message: `Expected tool call at index ${index}.`,
										})
									}
					...
					flush(controller) {
						...
						const modelMetadata = providerMetadata[providerOptionsName] ?? {}
						providerMetadata[providerOptionsName] = modelMetadata
						if (usage.completionTokensDetails.acceptedPredictionTokens != null) {
							modelMetadata.acceptedPredictionTokens =
								usage.completionTokensDetails.acceptedPredictionTokens
						}
						if (usage.completionTokensDetails.rejectedPredictionTokens != null) {
							modelMetadata.rejectedPredictionTokens =
								usage.completionTokensDetails.rejectedPredictionTokens
						}
