import type { AgentEvent, ModelTokenUsage, TokenUsage } from '@humanlayer/agentlayer-core'
		...
		if (msg.role === 'assistant') {
			...
			for (const part of parts) {
				if (part.type === 'text' && (part as { text: string }).text.trim()) {
					...
					const tc = part as { toolName: string; input: unknown; toolCallId?: string }
					if (eventHasAgentId(msg)) {
						void tc.toolCallId
					}
...
function eventHasAgentId(_msg: ModelMessage): boolean {
	return false
}
